...
Recent file-processing upgrades:

- The web UI now accepts PDFs in addition to images and text-like files.
- Pasted GitHub repo/file URLs and direct PDF URLs are pulled into model context automatically when possible.
- PDFs are sent as native `input_file` blocks instead of being flattened into plain text.
- GitHub-relevant turns can now expose explicit repo listing, repo search, and file-reading tools to the model.
- New binary attachments are stored under the local attachment store instead of being embedded directly in message JSON (`CHATBOT_ATTACHMENTS_DIR` can override the storage location).
- Uploaded text attachments and PDFs can now be indexed into a persistent OpenAI vector store and exposed back to the model through `file_search`.
